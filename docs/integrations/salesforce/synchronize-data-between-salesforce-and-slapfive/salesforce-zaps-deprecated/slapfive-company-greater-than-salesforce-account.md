---
description: >-
  This Zap keeps a Salesforce Account up to date when changes are made to a
  corresponding SlapFive Company.
---
...
### 1. Trigger to listen for a new or changed Company in SlapFive:
...
### 2. Action to look up the Salesforce Account ID.

This step is needed if you don't reliably maintain the Salesforce Account ID on the SlapFive Company records. It uses the Company Name to find a matching Salesforce Account.
...
4. On the **Set up action** screen, in the **Salesforce Object** field, select **Account**, and in the **Field to Search By** field, select **Account  Name**. In the **Search Value** field, select **1. Name: ...** and click **Continue**.
...
3. On the **Choose account** screen, select your Connected Account for Salesforce and click **Continue**.
4. On the **Set up action** screen, in the **Salesforce Object** field, select **Account**, and in the **Record to Update** field, click the **Custom** tab and then select the **2. ID ...** field.
...
6. On the **Test action** screen, click **Test & Review** to test the Zap. Go into Salesforce to verify that the Account record was updated properly.
